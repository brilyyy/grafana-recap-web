...
BEGIN
  ...
  DECLARE v_api_list TEXT DEFAULT 'SelectBankAccountType,UploadIdentityCard,RequestOpenAccount,RequestEmailOTP,VerifyEmail,RequestMobilePhoneOTP,VerifyMobilePhone,SubmitCustomerData,SaveCardInfo,RegisterEChannel,ChooseKYCMethod,VerifyLivenessResult';

  DECLARE cur_olob_data CURSOR FOR
    SELECT tanggal_transaksi, status_code, jumlah, api_name
    FROM tmp_olob_results;
  ...
  SELECT id INTO v_app_id FROM app_identifier WHERE app_name = v_app_name COLLATE utf8mb4_unicode_ci LIMIT 1;
  ...
  DROP TEMPORARY TABLE IF EXISTS tmp_olob_results;
  CREATE TEMPORARY TABLE tmp_olob_results (
  ...
  WHILE v_idx < 12 DO
    SET v_idx = v_idx + 1;
    SET v_api_name = TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(v_api_list, ',', v_idx), ',', -1));

    SET v_sql = CONCAT(
      'INSERT INTO tmp_olob_results (tanggal_transaksi, status_code, jumlah, api_name) ',
      'SELECT DATE(log_dt), SUBSTRING(log_msg, POSITION(''status_code'' IN log_msg), 18), ',
      'COUNT(SUBSTRING(log_msg, POSITION(''status_code'' IN log_msg), 18)), ',
      QUOTE(v_api_name),
      ' FROM `db_olob`.`openaccount_syslog` ',
      'WHERE log_dt >= ', QUOTE(v_start_timestamp),
      ' AND log_dt < ', QUOTE(DATE_ADD(v_processing_date, INTERVAL 1 DAY)),
      ' AND log_msg LIKE ', QUOTE(CONCAT('%', v_api_name, '%')),
  ...
  read_loop: LOOP
    FETCH cur_olob_data INTO v_tanggal_transaksi, v_status_code, v_jumlah, v_api_name;
    IF v_done THEN LEAVE read_loop; END IF;

    SET v_records_processed = v_records_processed + 1;
    SET v_rc = NULLIF(TRIM(v_status_code), '');
    ...
    SET v_is_success = (v_normalized_rc_desc IN ('sukses', 'success', 'berhasil'));
    ...
    IF NOT v_is_rc_empty AND v_api_name IS NOT NULL THEN
      ...
      WHERE id_app_identifier = v_app_id
        AND jenis_transaksi COLLATE utf8mb4_unicode_ci = v_api_name COLLATE utf8mb4_unicode_ci
    ...
    INSERT INTO app_success_rate (
      ...
      v_jumlah, 0, 0, NULL, v_error_type
    );
