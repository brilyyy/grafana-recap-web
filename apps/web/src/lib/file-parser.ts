...
export function parseDateValue(value: unknown): Date | null {
  ...
  if (typeof value === 'string') {
    ...
    if (slash.length === 3) {
      ...
      if (!Number.isNaN(d) && !Number.isNaN(m) && !Number.isNaN(y)) return buildValidDate(y, m, d)
    }
    ...
    if (dash.length === 3) {
      ...
      if (!Number.isNaN(y) && !Number.isNaN(m) && !Number.isNaN(d)) return buildValidDate(y, m, d)
    }
