...
export const appMappingsRelations = relations(appMappings, ({ one }) => ({
  app: one(appIdentifier, {
    fields: [appMappings.idAppIdentifier],
    references: [appIdentifier.id],
  }),
}))
...
export type NewAppMapping = typeof appMappings.$inferInsert
