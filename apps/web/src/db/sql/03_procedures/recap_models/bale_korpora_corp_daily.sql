...
recap_kind: bale_korpora_corp_daily
title: Bale Korpora — daily recap by ACTN_BY_CUST_ID (dimensional)
output_table: recap_bale_korpora_corp_daily
function_name: sp_recap_bale_korpora_corp_daily
scope_type: fixed_app
app_key: bale_korpora
raw_sql_repo_path: src/db/sql/reference/recap_models/bale_korpora_corp_daily.raw.sql
description: Aggregates Bale Korpora activity log by corporation, jenis transaksi, RC, status, and error_type per day into recap_bale_korpora_corp_daily.
brief_process_summary: For the processing date, deletes prior Bale Korpora rows for that day in recap_bale_korpora_corp_daily, rolls up bale_korpora_db_GCM_AGCM_LOG_ACTV one row per corporation (ACTN_BY_CUST_ID), day, jenis, RC, RC description, and IS_ERR—aligned with Bale Korpora daily success rate plus corp_id. error_type matches sp_process_bale_korpora_daily (dictionary, unmapped_rc, normalized rc).
brief_query: FROM "bale_korpora_db_GCM_AGCM_LOG_ACTV" a
...
→ COUNT(DISTINCT ID), SUM(AMT). Full SQL: src/db/sql/reference/recap_models/bale_korpora_corp_daily.raw.sql.
@endmeta */
CREATE OR REPLACE FUNCTION public.sp_recap_bale_korpora_corp_daily(p_processing_date DATE DEFAULT NULL)
RETURNS void AS $$
DECLARE
  v_app_id INT;
  v_app_name VARCHAR(255) := 'Bale Korpora';
...
BEGIN
  ...
  IF v_app_id IS NULL THEN
    RAISE EXCEPTION 'Application Bale Korpora not found in app_identifier table';
  END IF;

  INSERT INTO app_processing_log (
    app_name, id_app_identifier, processing_date, start_time, status, recap_kind, catalog_entry_id
  ) VALUES (
    v_app_name, v_app_id, v_processing_date, NOW(), 'running', 'bale_korpora_corp_daily', 'bale_korpora_corp_daily'
  )
  RETURNING id INTO v_log_id;

  BEGIN
    DELETE FROM recap_bale_korpora_corp_daily
    ...
    -- sp_process_bale_korpora_daily plus COALESCE(ACTN_BY_CUST_ID) for per-corporation daily rows.
    FOR rec IN
      SELECT *
      FROM (
        SELECT
      ...
      ORDER BY bale_korpora_corp_rollup.corp_id, bale_korpora_corp_rollup.jenis_transaksi
    LOOP
      ...
      INSERT INTO recap_bale_korpora_corp_daily (
        id_app_identifier,
        tanggal_transaksi,
        corp_id,
        jenis_transaksi,
