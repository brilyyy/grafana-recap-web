...
title: Debit Online — success rate (daily)
output_table: app_success_rate
function_name: sp_process_debit_online_daily
scope_type: per_app
app_key: debit_online
raw_sql_repo_path: src/db/sql/reference/success_rate/debit_online.raw.sql
description: H-1 recap of transaction success metrics into app_success_rate for Debit Online.
...
brief_query: FROM ASID160448_ZTRANS0P + ZRSPCD0P (FDW): TRTRTY=21, TRPCCD=59; INSERT app_success_rate.
@endmeta */
-- Debit Online: reads from ASID160448_ZTRANS0P + ASID160448_ZRSPCD0P (via FDW from itm_db)
-- Aligns with scripts/success_rate/debit_online/raw.postgres.sql:
--   TRTRTY = '21', TRPCCD = 59, TRXMDT encoded from processing date
CREATE OR REPLACE FUNCTION public.sp_process_debit_online_daily(p_processing_date DATE DEFAULT NULL)
RETURNS void AS $$
DECLARE
  v_app_id INT;
  v_app_name VARCHAR(255) := 'Debit Online';
  ...
  v_jenis_transaksi VARCHAR(255) := 'Debit Online';
  v_rc VARCHAR(50);
...
BEGIN
  ...
  IF v_app_id IS NULL THEN
    RAISE EXCEPTION 'Application Debit Online not found in app_identifier table';
  END IF;
