...
title: EDC Agen — success rate (daily)
output_table: app_success_rate
function_name: sp_process_edc_agen_daily
scope_type: per_app
app_key: edc_agen
raw_sql_repo_path: src/db/sql/reference/success_rate/edc_agen.raw.sql
description: H-1 recap of transaction success metrics into app_success_rate for EDC Agen.
...
brief_query: FROM ZTRANS0P (ITM/FDW): aggregate by TRXMDT / service; INSERT app_success_rate.
@endmeta */
-- EDC Agen: reads from ASID160448_ZTRANS0P + ASID160448_ZRSPCD0P (via FDW from itm_db)
-- Filter: TRTRTY NOT IN ('21','6A','6B','C1','C2'), TRPROD = 'POS'
CREATE OR REPLACE FUNCTION public.sp_process_edc_agen_daily(p_processing_date DATE DEFAULT NULL)
RETURNS void AS $$
DECLARE
  v_app_id INT;
  v_app_name VARCHAR(255) := 'EDC Agen';
...
BEGIN
  ...
  IF v_app_id IS NULL THEN
    RAISE EXCEPTION 'Application EDC Agen not found in app_identifier table';
  END IF;
  ...
  VALUES (v_app_name, v_app_id, v_processing_date, NOW(), 'running', 'sr:edc_agen')
  ...
  BEGIN
    ...
    FOR rec IN
      ...
      WHERE t."TRTRTY" NOT IN ('21', '6A', '6B', 'C1', 'C2')
        AND t."TRPROD" = 'POS'
