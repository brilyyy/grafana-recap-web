...
title: Bale Korpora — success rate (daily)
output_table: app_success_rate
function_name: sp_process_bale_korpora_daily
scope_type: per_app
app_key: bale_korpora
raw_sql_repo_path: src/db/sql/reference/success_rate/bale_korpora.raw.sql
description: H-1 recap of transaction success metrics into app_success_rate for Bale Korpora.
...
brief_query: FROM raw_bale_korpora: aggregate; INSERT app_success_rate.
@endmeta */
CREATE OR REPLACE FUNCTION public.sp_process_bale_korpora_daily(p_processing_date DATE DEFAULT NULL)
RETURNS void AS $$
DECLARE
  v_app_id INT;
...
BEGIN
  ...
  VALUES (v_app_name, v_app_id, v_processing_date, NOW(), 'running', 'sr:bale_korpora')
  ...
  BEGIN
    ...
    FOR rec IN
      SELECT
        ...
        0::DECIMAL(20,2)                        AS "Total Biaya Admin",
        CASE
