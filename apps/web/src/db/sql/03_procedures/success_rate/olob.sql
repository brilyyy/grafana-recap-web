...
title: OLOB — success rate (daily)
output_table: app_success_rate
function_name: sp_process_olob_daily
scope_type: per_app
app_key: olob
raw_sql_repo_path: src/db/sql/reference/success_rate/olob.raw.sql
description: H-1 recap of transaction success metrics into app_success_rate for OLOB.
...
brief_query: FROM raw_olob: aggregate by day, jenis, RC; INSERT app_success_rate.
@endmeta */
      ...
      LOOP
        ...
        IF NOT v_is_rc_empty AND v_jenis_transaksi IS NOT NULL THEN
          ...
          IF v_error_type IS NULL THEN
            ...
            VALUES (v_app_id, v_jenis_transaksi, COALESCE(v_normalized_rc, ''), COALESCE(v_rc_description, ''), v_rc, NULL)
