...
const MANAGED_COLUMNS: ManagedColumn[] = [
  ...
  {
    table: 'raw_table_housekeeping',
    column: 'date_column_type',
    type: 'VARCHAR(50)',
    default: "'timestamp'",
  },
...
async function migrateExistingProcedures(db: EngineDb, log: LogSink) {
  const { execSql, tableExists } = bind(db)
...
export async function diagnose(db: EngineDb): Promise<DiagnoseReport> {
  ...
  for (const e of objects.enums) {
    rows.push({
      category: 'enum',
      name: e,
      status: (await pgEnumExists(e)) ? 'ok' : 'missing',
    })
  }
  ...
  for (const { table, name } of objects.indexes) {
    ...
    rows.push({
      category: 'index',
      name,
      status: (await indexExists(table, name)) ? 'ok' : 'missing',
    })
  }
  ...
  for (const f of objects.functions) {
    rows.push({
      category: 'function',
      name: `${f}()`,
      status: (await functionExists(f)) ? 'ok' : 'missing',
    })
  }
  ...
  try {
    ...
    rows.push({
      ...
      name: 'scheduler_jobs',
      status: 'error',
      detail: 'scheduler_jobs table absent',
    })
  }
