...
def _run(stdscr: curses.window, path: Path) -> None:
    ...
    while True:
        ...
        if key in (ord("q"), ord("Q"), 27):
            ...
            curses.doupdate()
            # Use --print inspect as a substitute for the old generate action
            try:
                result = subprocess.run(
                    [sys.executable, "-m", "bptx._debug", str(path), "--print"],
                    capture_output=True,
                    text=True,
                )
...
def main(path: Optional[str] = None) -> None:
    ...
    if not path:
        print("Usage: python -m bptx._debug <path/to/template.pptx>", file=sys.stderr)
        sys.exit(1)
    target = Path(path)
