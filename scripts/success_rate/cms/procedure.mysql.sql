CREATE PROCEDURE sp_process_cms_daily(IN p_processing_date DATE)
MODIFIES SQL DATA
SQL SECURITY DEFINER
BEGIN
  DECLARE v_app_id INT;
  DECLARE v_app_name VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'CMS';
  ...
  DECLARE cur_cms_data CURSOR FOR
    SELECT
  ...
  SET v_done = 0;
  IF v_app_id IS NULL THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Application CMS not found in app_identifier table'; END IF;
  ...
  OPEN cur_cms_data;
  read_loop: LOOP
    FETCH cur_cms_data INTO v_tanggal_transaksi, v_jenis_transaksi, v_rc, v_rc_description,
