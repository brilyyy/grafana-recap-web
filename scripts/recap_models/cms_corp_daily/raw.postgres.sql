-- Representative aggregation for CMS recap by ACTN_BY_CUST_ID (corp_id): same split as
-- scripts/success_rate/cms/raw.postgres.sql (jenis, RC, RC Description, Status via IS_ERR),
-- plus one grouping column for corporation so each row is a per-corp daily slice of activity.
...
ORDER BY cms_corp_rollup.corp_id, cms_corp_rollup."Jenis Transaksi";
