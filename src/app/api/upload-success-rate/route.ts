...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file) {
      return NextResponse.json({ success: false, message: 'No file uploaded' } as ApiResponse, { status: 400 })
    }

    if (!selectedApplicationId || isNaN(parseInt(selectedApplicationId))) {
      return NextResponse.json({ success: false, message: 'Valid application selection is required' } as ApiResponse, { status: 400 })
    }
    ...
    if (isCSV) {
      ...
      if (rows.length === 0) {
        return NextResponse.json({ success: false, message: 'CSV file is empty' } as ApiResponse, { status: 400 })
      }
      ...
      if (missingColumns.length > 0) {
        return NextResponse.json({ success: false, message: `Missing required columns: ${missingColumns.join(', ')}` } as ApiResponse, { status: 400 })
      }
      ...
      for (let rowNum = 1; rowNum < rows.length; rowNum++) {
        ...
        // Business rule: empty RC + sukses → RC='00'
      ...
      if (workbook.SheetNames.length === 0) {
        return NextResponse.json({ success: false, message: 'Excel file contains no worksheets' } as ApiResponse, { status: 400 })
      }
    ...
    if (successRateData.length === 0) {
      return NextResponse.json({ success: false, message: 'No valid success rate data found in the file' } as ApiResponse, { status: 400 })
    }

    // Verify application exists
    const appResult = await db.execute(sql`SELECT app_name FROM app_identifier WHERE id = ${applicationId}`)
    if (appResult.rows.length === 0) {
      return NextResponse.json({ success: false, message: 'Selected application does not exist' } as ApiResponse, { status: 400 })
    }
    const applicationName = (appResult.rows[0] as any).app_name

    // Use Drizzle transaction
    await db.transaction(async (tx) => {
      // Lookup error_type from dictionary for each entry
    ...
    if (error.message?.includes('Unauthorized') || error.message?.includes('Forbidden')) {
      return NextResponse.json({ success: false, message: error.message } as ApiResponse, { status: 403 })
    }
