...
const IS_COMPILED =
  import.meta.url.endsWith(".mjs") || import.meta.url.endsWith(".js");
...
function startSchedulerWorker() {
  ...
  schedulerWorker.on("message", (msg: any) => {
    if (msg?.type === "ready") {
      restartAttempts = 0;
      console.log(
        `[server] Scheduler worker ready (pid=${schedulerWorker?.pid}, jobs=${msg.jobCount})`,
      );
    }
  });
  ...
  schedulerWorker.on("exit", (code, signal) => {
    console.warn(
      `[server] Scheduler worker exited (code=${code}, signal=${signal})`,
    );
