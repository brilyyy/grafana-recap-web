      ...
      <header className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Read-only index health from PostgreSQL runtime stats. Recommendations are copyable SQL — nothing is executed.
            {report ? (
              <>
                {' '}
      ...
      {query.isError && (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>{query.error?.message ?? 'Could not read index statistics.'}</EmptyDescription>
          </EmptyHeader>
        </Empty>
      )}
      ...
      {report && (
        <>
          <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-5">
            ...
            <StatCard label="Drift: missing" value={String(report.summary.driftMissing)} hint="defined, not in DB" />
            <StatCard label="Drift: extra" value={String(report.summary.driftExtra)} hint="in DB, not in source" />
          </div>

          <SectionCard
            title="Unused indexes"
            description="idx_scan = 0 since stats reset. Excludes primary/unique. Safe drop candidates."
          >
          ...
          <SectionCard title="Sizes & bloat" description="Largest relations. Dead-tuple % ≥ 20 suggests a VACUUM.">
          ...
          <SectionCard
            ...
            description="Indexes defined in src/db/sql/02_indexes but missing in the DB, or present in the DB but not in source."
          >
