...
function parseCSV(text: string): string[][] {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (char === '"') {
      if (inQuotes && nextChar === '"') { currentLine += '"'; i++ }
  ...
  return lines.map(line => {
    ...
    for (let i = 0; i < line.length; i++) {
      const char = line[i]
      if (char === '"') {
        if (inFieldQuotes && line[i + 1] === '"') { currentField += '"'; i++ }
        else inFieldQuotes = !inFieldQuotes
      } else if (char === ',' && !inFieldQuotes) { fields.push(currentField.trim()); currentField = '' }
    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          let dictionaryData: Array<{ jenis_transaksi: string; rc: string; rc_description: string | null; error_type: 'S' | 'N' | 'Sukses' }> = []
          const skippedRows: Array<{ rowNumber: number; reason: string }> = []

          if (isCSV) {
            const text = await file.text()
            const rows = parseCSV(text)
            if (rows.length === 0) return Response.json({ success: false, message: 'CSV file is empty' }, { status: 400 })
            ...
            for (let i = 1; i < rows.length; i++) {
              ...
              if (!et) { skippedRows.push({ rowNumber: i + 1, reason: `Invalid S/N: "${sn}"` }); continue }
              dictionaryData.push({ jenis_transaksi: jt, rc, rc_description: desc || null, error_type: et })
            }
          } else {
            const bytes = await file.arrayBuffer()
            const workbook = XLSX.read(Buffer.from(bytes), { type: 'buffer' })
            if (workbook.SheetNames.length === 0) return Response.json({ success: false, message: 'No worksheets' }, { status: 400 })
            ...
            for (let r = 1; r <= range.e.r; r++) {
              const jt = ws[XLSX.utils.encode_cell({ r, c: jtIdx })]?.v ? String(ws[XLSX.utils.encode_cell({ r, c: jtIdx })].v).trim() : ''
              const rc = ws[XLSX.utils.encode_cell({ r, c: rcIdx })]?.v ? String(ws[XLSX.utils.encode_cell({ r, c: rcIdx })].v).trim() : ''
              const sn = ws[XLSX.utils.encode_cell({ r, c: snIdx })]?.v ? String(ws[XLSX.utils.encode_cell({ r, c: snIdx })].v).trim().toUpperCase() : ''
              const desc = descIdx >= 0 && ws[XLSX.utils.encode_cell({ r, c: descIdx })]?.v ? String(ws[XLSX.utils.encode_cell({ r, c: descIdx })].v).trim() : null
              if (!jt && !rc) continue
              let et: 'S' | 'N' | 'Sukses' | null = null
              if (sn === 'S') et = 'S'
              else if (sn === 'N') et = 'N'
              else if (sn === 'SUKSES' || sn === 'SUCCESS' || sn === 'BERHASIL') et = 'Sukses'
              if (!et) { skippedRows.push({ rowNumber: r + 1, reason: `Invalid S/N: "${sn}"` }); continue }
              dictionaryData.push({ jenis_transaksi: jt, rc, rc_description: desc || null, error_type: et })
            }
          }

          if (skippedRows.length > 0) return Response.json({ success: false, message: `${skippedRows.length} row(s) have errors`, data: { skippedRows } }, { status: 400 })
          if (dictionaryData.length === 0) return Response.json({ success: false, message: 'No valid data' }, { status: 400 })

          const appResult = await db.execute(sql`SELECT app_name FROM app_identifier WHERE id = ${applicationId}`)
          if (appResult.rows.length === 0) return Response.json({ success: false, message: 'Application not found' }, { status: 400 })
          ...
          await logAuditEvent(session.userId, session.username, 'DICTIONARY_UPLOADED', 'response_code_dictionary', applicationId.toString(), `Uploaded ${dictionaryData.length} entries for ${appName}`, getClientIp(request), getUserAgent(request))

          return Response.json({ success: true, message: `Dictionary uploaded. ${dictionaryData.length} entries processed.`, data: { entriesProcessed: dictionaryData.length, applicationId, applicationName: appName } })
        } catch (error: any) {
