...
export function prettifyZodError(error: ZodError, label?: string): string {
  const header = label ? `❌ Environment validation failed (${label}):\n` : '❌ Environment validation failed:\n'
  ...
  for (const issue of error.issues as ZodIssue[]) {
    ...
    switch (issue.code) {
      case 'invalid_type':
        ...
        break
      case 'too_small': {
        const origin = 'origin' in issue ? (issue as { origin: string }).origin : ''
        const qualifier = origin === 'string' ? ' length' : ''
        const min = 'minimum' in issue ? (issue as { minimum: number | bigint }).minimum : ''
      ...
      case 'too_big': {
        const origin = 'origin' in issue ? (issue as { origin: string }).origin : ''
        const qualifier = origin === 'string' ? ' length' : ''
        const max = 'maximum' in issue ? (issue as { maximum: number | bigint }).maximum : ''
      ...
      case 'invalid_format': {
        const fmt = 'format' in issue ? (issue as { format: string }).format : ''
        hint = fmt ? ` (format: ${fmt})` : ''
        break
      }
      default:
  ...
  if (isDev) {
    lines.push('   Run with SKIP_ENV_VALIDATION=true to bypass this check temporarily.')
  }
...
export interface CreateEnvOptions<TServerSchema extends EnvSchema, TClientSchema extends EnvSchema> {
 ...
 */
function filterAndStripPrefix(source: Record<string, unknown>, prefix: string): Record<string, unknown> {
 ...
 */
export function createEnv<TServerSchema extends EnvSchema, TClientSchema extends EnvSchema>(
  ...
  const rawServerSource: Record<string, unknown> =
    parseFrom.server ?? (typeof process !== 'undefined' ? (process.env as Record<string, unknown>) : {})
  const rawClientSource: Record<string, unknown> =
    parseFrom.client ?? (typeof import.meta.env !== 'undefined' ? (import.meta.env as Record<string, unknown>) : {})

  const filteredClientSource = filterAndStripPrefix(rawClientSource, clientPrefix)
  ...
  if (skipValidation) {
    ...
    clientEnv = parseEnv(clientSchema, transformClient(filteredClientSource), 'client', onValidationError)
  } else {
    serverEnv = parseEnv(serverSchema, transformServer(rawServerSource), 'server', onValidationError)
    clientEnv = parseEnv(clientSchema, transformClient(filteredClientSource), 'client', onValidationError)
  }

  const merged = { ...serverEnv, ...clientEnv } as ZodInfer<TServerSchema> & ZodInfer<TClientSchema>
  ...
  const handler: ProxyHandler<typeof merged> = {
    get(target, prop, receiver) {
      ...
      if (!isServer && runtimeCheckServerAccess && serverSchemaKeys.has(key) && !clientSchemaKeys.has(key)) {
        throw new Error(
    ...
    set() {
      if (isDev) {
        console.warn('[env] Attempted to set environment variable — ignored (read-only).')
      }
      return true
    },
    ownKeys(target) {
