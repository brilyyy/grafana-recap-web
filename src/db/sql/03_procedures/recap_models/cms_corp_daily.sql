...
output_table: recap_cms_corp_daily
function_name: sp_recap_cms_corp_daily
scope_type: fixed_app
app_key: cms
raw_sql_repo_path: src/db/sql/reference/recap_models/cms_corp_daily.raw.sql
description: Aggregates CMS activity log by corporation, jenis transaksi, RC, status, and error_type per day into recap_cms_corp_daily.
brief_process_summary: For the processing date, deletes prior CMS rows for that day in recap_cms_corp_daily, rolls up cms_db_GCM_AGCM_LOG_ACTV one row per corporation (ACTN_BY_CUST_ID), day, jenis, RC, RC description, and IS_ERR—aligned with CMS daily success rate plus corp_id. error_type matches sp_process_cms_daily (dictionary, unmapped_rc, normalized rc).
brief_query: FROM "cms_db_GCM_AGCM_LOG_ACTV" a
...
→ COUNT(DISTINCT ID), SUM(AMT). Full SQL: src/db/sql/reference/recap_models/cms_corp_daily.raw.sql.
@endmeta */
