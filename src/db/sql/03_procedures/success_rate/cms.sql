...
title: CMS — success rate (daily)
output_table: app_success_rate
function_name: sp_process_cms_daily
scope_type: per_app
app_key: cms
raw_sql_repo_path: src/db/sql/reference/success_rate/cms.raw.sql
description: H-1 recap of transaction success metrics into app_success_rate for CMS.
...
brief_query: FROM cms_db_GCM_AGCM_LOG_ACTV: GROUP BY day, service, ERR_MAP, IS_ERR; INSERT app_success_rate.
@endmeta */
CREATE OR REPLACE FUNCTION public.sp_process_cms_daily(p_processing_date DATE DEFAULT NULL)
RETURNS void AS $$
DECLARE
  v_app_id INT;
