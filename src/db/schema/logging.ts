...
export const appProcessingLog = pgTable(
  ...
  (t) => ({
    appDateIdx: index("idx_app_processing_date").on(
      t.appName,
      t.processingDate,
    ),
    statusIdx: index("idx_apl_status").on(t.status, t.createdAt),
    processingDateIdx: index("idx_app_processing_log_processing_date").on(
      t.processingDate,
    ),
    catalogEntryDateIdx: index("idx_apl_catalog_entry_date").on(
      t.catalogEntryId,
      t.processingDate,
    ),
  }),
